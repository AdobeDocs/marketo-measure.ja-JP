...
デフォルトでは、Dynamics には推定クローズ日と実際のクローズ日の 2 つの在庫クローズ日フィールドがあるので、パイプラインの収益データはダッシュボードで使用できません。[!DNL Marketo Measure] は、ダッシュボード内の 1 つのクローズ日フィールドのみを指すことができ、実際のクローズ日を指しています。
