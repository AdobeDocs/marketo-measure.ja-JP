...
Hubspot は、ランディングページ/フォームや web サイトをホストできるという点で、他のマーケティング自動化システムとは少し異なります。 以下の手順は、[!DNL Hubspot] ーザーがホストするページでアクティビティ [!DNL Marketo Measure] 追跡するためのものです。 [!DNL Hubspot] 以外のCMS（Wordpress など）を使用して web サイトの機能を強化する場合は、[!DNL Marketo Measure] JavaScriptもそのCMSに追加する必要があります。
