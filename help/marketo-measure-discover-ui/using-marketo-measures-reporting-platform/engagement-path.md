...
**タッチポイント日：** オンラインソースの場合：エンゲージメントが発生した日時。 オフラインイベントの場合：Salesforce キャンペーンで設定された日時。 アクティビティタッチポイントの場合：アクティビティ設定で参照されているタッチポイント日付フィールド
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td><p>属性モデル</p></td> 
   <td><p>属性売上高を計算するモデルを指定します。 許可されている値：「フルパス属性」、「ファーストタッチ属性」、「カスタムモデル属性」、「リード作成属性」、「U 字形属性」、「W 字形属性」。</p></td> 
  </tr> 
  <tr> 
   <td><p>イベントタイプ</p></td> 
   <td><p>ユーザータッチポイントが基にしているイベントのタイプでジャーニーをフィルタリングします。 右側のプラス記号「+」を使用してフィルターを追加し、複数の値を許可します。 許可されている値："セッション"、"CRM キャンペーン"、"CRM イベント"、"CRM タスク"、"インプレッション"。</p></td> 
  </tr> 
  <tr> 
   <td><p>リードステージ</p></td> 
