---
description: Marketo Measure ユーザー向けのオンラインカスタムチャネル設定ガイダンス
...
* ファイルがアップロードされると、7 日間はルールを変更できません。[!DNL Marketo Measure] は、この時間を利用して、タッチポイントを処理および更新します。
...
![&#x200B; 最初の手順は、からカスタムチャネルのスプレッドシートをダウンロードすることです &#x200B;](assets/online-channels-1.png)

スプレッドシートには、次の 7 つの列があります。

![&#x200B; スプレッドシートには 7 つの列があります。](assets/online-channels-2.png)
...
* **参照元 web サイト：**&#x200B;ページへのトラフィックを参照する web サイトの URL またはビルトインの [!DNL Marketo Measure] 論理（角括弧で示される）

8 番目の列は、スプレッドシートから削除できないルールを「Do Not Remove」で示します。 スプレッドシートの上部にはデフォルトのチャネルルールがあり、[!DNL Marketo Measure] ではこれらのチャネルを使用しない場合でも、変更や削除を行わないことを推奨しています。[!DNL Marketo Measure] には、これらのプラットフォームとの深い統合があるので、デフォルトで含まれています。
...
![&#x200B; 行は、ルールとMarketo Measureの順序を表しています &#x200B;](assets/online-channels-3.png)
...
![Marketo Measureには、お使いのユーザーに応じた 12 のデフォルトのチャネルが用意されています。 これらは &#x200B;](assets/online-channels-4.png)
...
![&#x200B; ルールの各パラメーターまたはコンポーネントは、に個別にマッピングされます &#x200B;](assets/online-channels-5.png)
...
![&#x200B; 満たしていないデータをキャッチするために、デフォルトのチャネル「Other」が存在します &#x200B;](assets/online-channels-6.jpg)
