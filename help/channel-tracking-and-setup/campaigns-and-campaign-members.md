---
description: Marketo Measure ユーザー向けのキャンペーンとキャンペーンメンバーガイダンス
...
![Marketo Measure Salesforce パッケージには、というラベルの付いたフィールドが含まれます ](assets/dynamics-lists-1.png)

選択リストで使用できるオプションは次のとおりです。

![ 選択リストで使用できるオプションは次のとおりです。](assets/dynamics-lists-10.png)
...
![Marketo Measureを使用する前は、マーケティング自動処理プラットフォームを使用していました ](assets/dynamics-lists-2.png)
...
![ 有効にすると、Marketo Measureはステータス値をにプッシュします ](assets/dynamics-lists-3.png)
...
![ これは、リストがその後、日/週/月の後でアップロードされた場合に必要になる可能性があります ](assets/dynamics-lists-4.png)
...
![ 一括更新タッチポイント日は、インストールされたMarketoに含まれています ](assets/dynamics-lists-5.png)
...
![1 つの検索が機能せず、に表示される ](assets/legacy-processes-10.png)
...
![ 日時を設定したら、「選択項目を更新」をクリックします ](assets/dynamics-lists-6.png)

## キャンペーンコスト {#campaign-costs}

キャンペーンコストについて詳しくは、[この記事](/help/crm-campaign-costs.md){target="_blank"}を参照してください。
...
この問題を回避するために、[!DNL Marketo Measure] は、キャンペーンメンバーが削除されるたびにトラッキングする [!DNL Marketo Measure] ール履歴オブジェクトとトリガーを作成し、対応するタッチポイントを削除しました。**この機能 [!DNL Marketo Measure] 使用するには** Marketing Analytics パッケージ V6.15 以降が必要です。
