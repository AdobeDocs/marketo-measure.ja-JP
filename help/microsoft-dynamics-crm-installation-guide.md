---
description: Microsoft Dynamics CRM にMarketo Measure パッケージをインストールして設定するためのステップバイステップガイド
...
1. 「[!UICONTROL 資格情報]」または「[!UICONTROL OAuth]」を選択します。
   ...
   >[!NOTE]
   >
   >OAuth について詳しくは、[この記事](/help/oauth-with-azure-active-directory-for-dynamics-crm.md)を参照してください。プロセスについてご質問がある場合は、[!DNL Marketo Measure] アカウント担当者にお問い合わせください。
...
1. 次に、[[!DNL Marketo Measure] JavaScript](/help/marketo-measure-tracking/adding-marketo-measure-script.md) をサイト全体とランディングページ全体に配置する必要があります。ランディングページの先頭にスクリプトをハードコードするか、[Google タグマネージャー](/help/marketo-measure-tracking/adding-marketo-measure-script-via-google-tag-manager.md)などのタグ管理システムを通じて追加することをお勧めします。
