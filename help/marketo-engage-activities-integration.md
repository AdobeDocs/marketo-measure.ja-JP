...
1. Marketo アクティビティの設定を開始するには、**マイアカウント**/**設定**/**アクティビティ** に移動します。
   ...
   ![1. Marketo アクティビティの設定を開始するには、「マイアカウント設定」に移動します ](assets/marketo-engage-activities-01.png)

   ![1. Marketo アクティビティの設定を開始するには、「マイアカウント設定」に移動します ](assets/marketo-engage-activities-02.png)
...
1. [!UICONTROL  アクティビティタイプを選択 ] の下のドロップダウンメニューをクリックして、様々なタイプの選択を開始します。
   ...
   ![1. 開始するアクティビティタイプを選択で、ドロップダウンメニューをクリックし ](assets/marketo-engage-activities-03.png) す。

1. 必要なアクティビティをすべて選択したら、それらのアクティビティが [!UICONTROL  選択したアクティビティリスト ] および [!UICONTROL  ルールを定義 ] に表示されます。

   ![1. 必要なアクティビティをすべて選択すると、](assets/marketo-engage-activities-04.png) の情報が表示されます。
...
1. まず、アクティビティタイプに応じて、後でチャネルマッピングに使用できる [!DNL Marketo Measure] キャンペーン名を設定する必要がある場合があります。[!DNL Marketo Measure] キャンペーン名は、複数のルールで再利用できます。 これにより、単一のチャネルルールで使用できる幅広い名前を付けることができます。 すべてのアクティビティタイプにMarketo プログラムが含まれているわけではないので、この最初の手順では名前が必要です。

   この追加の手順の例を次に示します。

   ![ その追加の手順の例を次に示します。](assets/marketo-engage-activities-05.png)
...
1. 「スコアを変更」の例では、キャンペーン名を入力する必要があります。キャンペーン名はMarketo プログラムから取り込まれるからです。 次に、ルール式を作成します。 次の例では、「[!UICONTROL  次よりも大きい ]」という演算子を持つ値 90 の「[!UICONTROL  新しい値 ]」フィールドを選択します。
   ...
   ![ ルールを展開して、追加のフィルターまたは条件を追加できます ](assets/marketo-engage-activities-06.png)

   ![ ルールを展開して、追加のフィルターまたは条件を追加できます ](assets/marketo-engage-activities-07.png)

1. 最後に、タッチポイント日として使用するものを選択します。 使用可能なすべての日付または日付/時間フィールドがMarketoからここに表示されます。 カスタム日付フィールドがない場合は、「[!UICONTROL  アクティビティ日 ]」と表示されます。

   ![1. 最後に、タッチポイント日として使用するものを選択します。 すべて利用可能 ](assets/marketo-engage-activities-08.png)

1. 変更が失われないように、途中で **[!UICONTROL ドラフトとして保存]** をクリックしてください。

   ![1. 途中で必ず「ドラフトとして保存」をクリックしてください ](assets/marketo-engage-activities-09.png)
...
プログラム ID を持つアクティビティタイプのルールのいずれについても、Marketo プログラムチャネルはプログラムから決定されます。 プログラムチャネルはカスタムのオフラインチャネルにマッピングするために使用されるので、チャネルが（ここで説明されているように [ 正しく設定されていることを確認する必要があ ](/help/marketo-engage-programs-integration.md) ます。

また、プログラム ID を持たないアクティビティタイプのルールに対しては、最初にキャンペーン名を作成しました。 このキャンペーン名を使用して、（ここにレイアウトされた [ カスタムオンラインチャネルを設定 ](/help/channel-tracking-and-setup/online-custom-channel-setup.md) ます。
...
Mark が Facebook 広告をクリックすると、wayneenterprises.comにアクセスし、[!DNL Marketo Measure] ID 123 および [!DNL Marketo Munchkin Id] 456 の Cookie を取得します。 フォームへの入力は行われません。
...
この cookie マッピングの機能強化により、FT は戻って Facebook 広告にクレジットされ、LC はメールにクレジットされます。
