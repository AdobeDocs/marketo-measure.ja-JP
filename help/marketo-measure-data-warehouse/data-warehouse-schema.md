...
_Data Warehouse データモデル_ ERD は、データウェアハウス内のデータが、どのように流れ、リンクされることを意図されているかを示します。テーブルの一部は、マッピングテーブル、既に存在している他のテーブルのビューまたは使用を推奨しない廃止されたテーブルを表しているので、この図には、データウェアハウスに含まれるすべてのテーブルが含まれるわけではありません。以下の Data Warehouse に含まれるテーブルと列の詳細な説明を参照してください。 これらのテーブルの多くには、非正規化されたフィールドが含まれていますが、この図は、代わりにディメンションテーブルからデータを活用する、推奨されるデータモデルです。
...
>[!IMPORTANT]
>
>Marketo Measure は、2024 年半ばにこのテーブルを廃止する予定です。自分の側で作成する場合は、を実行します。 [この SQL クエリ](/help/marketo-measure-data-warehouse/assets/BIZ_FACTS.sql).
