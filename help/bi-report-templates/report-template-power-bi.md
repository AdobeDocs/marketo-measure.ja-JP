...
ファイルを開く際に入力するパラメーターは、Data Warehouse からテーブルを読み込むネイティブクエリを構造化するために使用します。この場合も、[!DNL Snowflake] インスタンスへのデータ接続を設定する必要があります。このためには、ユーザ名とパスワードと共に同じサーバー名とウェアハウス名が必要です。ユーザー名を見つけて、必要に応じてパスワードをリセットする場所の詳細は、[こちら](/help/data-warehouse/data-warehouse-access-reader-account.md){target="_blank"}に記載されています。
