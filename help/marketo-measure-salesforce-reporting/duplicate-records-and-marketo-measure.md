---
description: Marketo Measure ユーザー向けの重複  [!DNL Marketo Measure]  るレコードとガイダンス
...
![ サンプルレポート：購入者タッチポイントを持つMarketo Measure人物 ](assets/additional-functionality-1.png)
...
* レポートに対する収益を最大化するために、CRM 内で重複排除ツールを使用して、新しい一意のレコードのみを確実に作成することをお勧めします。 これは、マーケティング自動化ツールまたは CRM 内にインストールされた別のソフトウェアで行うことができます。[!DNL Marketo Measure] は記録を自動的に重複排除せず、当社のソフトウェアを通じてこのサービスを提供しません。
